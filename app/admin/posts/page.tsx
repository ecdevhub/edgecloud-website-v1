...
export default async function AdminPostsPage({
  ...
  if (searchParams.search)
    conditions.push(or(like(schema.posts.title, `%${searchParams.search}%`)));

  const where = conditions.length > 0 ? and(...conditions) : undefined;
  const [rows, [{ count: total }]] = await Promise.all([
    db
      .select({ post: schema.posts, author: schema.authors, category: schema.categories })
      .from(schema.posts)
  ...
  const statusCounts = await Promise.all(
    ["published", "draft", "archived"].map(async (s) => {
      const [r] = await db
        .select({ count: sql<number>`count(*)` })
        .from(schema.posts)
        .where(eq(schema.posts.status, s as any));
    ...
    <AdminShell user={user}>
      <div style={{ padding: "32px 36px" }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 14, color: "#8B96A3", margin: 0 }}>
              {total} total · Manage your blog content
            </p>
          </div>
          <Link
        ...
        >
          ...
          >
            <span style={{ padding: "0 10px", color: "#8B96A3", display: "flex" }}>
              <Search size={14} />
            </span>
            <input
              name="search"
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[
              { label: "All", status: "" },
              ...statusCounts.map((s) => ({
                label: `${s.status.charAt(0).toUpperCase() + s.status.slice(1)} (${s.count})`,
                status: s.status,
              })),
            ].map(({ label, status }) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    fontSize: 13,
                    textDecoration: "none",
                    background: active ? "#0F1923" : "white",
                    color: active ? "white" : "#4A5568",
                    border: "1.5px solid",
                    borderColor: active ? "#0F1923" : "#E5E8ED",
                  }}
                >
                  {label}
                </Link>
        ...
        >
          {rows.length === 0 ? (
            <div style={{ padding: "64px 24px", textAlign: "center" }}>
              <FileText
                size={32}
                style={{ color: "#CDD2D9", margin: "0 auto 12px", display: "block" }}
              />
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {rows.map(({ post, author, category }, i) => (
                  <tr
                    key={post.id}
                    style={{ borderBottom: i < rows.length - 1 ? "1px solid #F4F6F8" : "none" }}
                  >
                      ...
                      {category ? (
                        <span
                          style={{
                            ...
                            background: `${category.color ?? "#00A2FF"}15`,
                            padding: "3px 8px",
                            borderRadius: 999,
                          }}
                        >
                        ...
                        <span style={{ color: "#CDD2D9", fontSize: 12 }}>-</span>
                      )}
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                      <span
                    ...
                    <td style={{ padding: "14px 16px", fontSize: 13, color: "#4A5568" }}>
                      <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <Eye size={12} style={{ color: "#8B96A3" }} />{" "}
        ...
        {pages > 1 && (
          <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 24 }}>
            {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
              <Link
                key={p}
                href={`/admin/posts?page=${p}${searchParams.status ? `&status=${searchParams.status}` : ""}`}
                style={{
                  width: 36,
                  height: 36,
                  display: "flex",
                  ...
                  fontSize: 14,
                  textDecoration: "none",
                  background: p === page ? "#00A2FF" : "white",
                  color: p === page ? "white" : "#4A5568",
                  border: "1.5px solid",
                  borderColor: p === page ? "#00A2FF" : "#E5E8ED",
                }}
              >
                {p}
              </Link>
